                    ...
                    <tr id="row<?= $blog['id'] ?>">
                        ...
                        <td>
                            <?php
                                ...
                                $content = strip_tags($blog['content']); // Remove HTML tags if any
                        ...
                        <td>
                            <img src="<?= base_url('uploads/blogs/' . $blog['image']) ?>" alt="Main Image" style="width: 100px; height: 100px;">
                        </td>

                        <td>
                            <div id="msg<?= $blog['id'] ?>" class="text-center">
                                <span style="color: <?= $blog['status'] == 1 ? 'green' : 'red' ?>;">
                                    <b><?= $blog['status'] == 1 ? 'ACTIVE' : 'INACTIVE' ?></b>
                                </span>
                            </div>
                            <label class="switch">
        ...
        document.querySelectorAll('.change_status').forEach(element => {
            element.addEventListener('click', function () {
                ...
                if (confirm("Are you sure you want to change the status?")) {
                    fetch("<?= site_url('webmaster/viewstatus') ?>", {
                        method: "POST",
                        headers: {
