                    ...
                    <tr id="row<?= $desk['id'] ?>">
                        ...
                        <td>
                            <?php
                                ...
                                $content = strip_tags($desk['content']); // Remove HTML tags if any
                        ...
                        <td>
                            <img src="<?= base_url('uploads/traveldesk/' . $desk['image']) ?>" alt="Main Image" style="width: 100px; height: 100px;">
                        </td>

                        <td>
                            <div id="msg<?= $desk['id'] ?>" class="text-center">
                                <span style="color: <?= $desk['status'] == 1 ? 'green' : 'red' ?>;">
                                    <b><?= $desk['status'] == 1 ? 'ACTIVE' : 'INACTIVE' ?></b>
                                </span>
                            </div>
                            <label class="switch">
                                <input type="checkbox" class="switch-input change_status" 
                                       id="<?= $desk['id'] ?>" 
                                       <?= $desk['status'] == 1 ? 'checked' : '' ?>>
                                <span class="switch-label" data-on="Active" data-off="Deactive"></span>
                                <span class="switch-handle"></span>
                            </label>
                        </td>
                        <td>
                            <a class="btn btn-primary btn-sm" href="<?= base_url('webmaster/desk/edit/' . ($desk['id'] ?? '')); ?>"><span class="fa fa-pencil"></span> Edit</a><br>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        ...
        document.querySelectorAll('.change_status').forEach(element => {
            element.addEventListener('click', function () {
                ...
                if (confirm("Are you sure you want to change the status?")) {
                    fetch("<?= site_url('webmaster/viewsdeskstatus') ?>", {
                        method: "POST",
                        headers: {
